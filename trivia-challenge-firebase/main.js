...
// Load trivia data and populate dropdown
fetch("./trivia.json")
  .then(res => res.json())
  .then(data => {
    triviaData = data;
...
startBtn.addEventListener("click", () => {
  ...
  if (!name) {
    alert("Please enter a name or tag!");
    return;
  }
...
async function showResults() {
  ...
  const message = score === currentQuestions.length ? "Perfect Score!" :
                  score === 0 ? "Oof. Try again!" :
                  score < currentQuestions.length / 2 ? "Not bad, give it another go!" :
                  "Well done!";
